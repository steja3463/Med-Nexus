    ...
    <div className="m-5">
      ...
      {appointments.length > 0 ? (
        appointments.map((appointment) => (
          <div
            key={appointment._id}
            className="h-16 flex justify-between items-center rounded-lg gap-8 mt-4 bg-slate-200"
          >
            <p className="ml-4">
              Notification received by {appointment.patientId.userName} to{" "}
              {appointment.doctorId.name} at time:{" "}
